  ...
  unsigned size, bool aligned, u8 **tr_ptr)
{
  ...
  if (meminfo->check_smc && !aligned) {
    /* ADR-0030: publish the exact lui constant this stub loads into reg_rv.
     * When the SMC branch below is taken, reg_rv holds (this constant + the
     * masked region offset) and reg_a0 holds the offset, so smc_write can hand
     * both to smc_prof_note_store() and it can name the region without
     * re-deriving any address arithmetic.  Must stay identical to the
     * mips_emit_lui() below it (which masks the immediate to 16 bits).
     * Counters only; no emitted code. */
    u32 luibase = (((base_addr + 0x8000) >> 16) & 0xFFFF) << 16;
    if (region == 2) {
      smc_prof_lui_ewram = luibase;
      mips_emit_lui(reg_temp, 0x40000 >> 16);
      mips_emit_addu(reg_temp, reg_rv, reg_temp); // SMC lives after the ewram
    } else {
      smc_prof_lui_iwram = luibase;
