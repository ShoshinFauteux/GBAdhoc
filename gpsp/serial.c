...
#include "common.h"
#include "main.h"   /* ADR-0051: core_phase probe */
  ...
  case SERIAL_MODE_NORMAL:
    ...
    if (serial_mode == SERIAL_MODE_RFU) {
      ...
      if ((newval & 0x0080) && (newval & 0x1) && !serial_irq_cycles) {
        ...
        /* ADR-0051: this is the adapter doing real work, and it runs inside
         * translated code, so its cost lands in the `cpu` residue where it is
         * invisible. Counted always (one increment per SIO word); timed at
         * CORE_PHASE_FINE. See main.h. */
        u32 rval;
        u32 cph_t = core_phase_enter(CORE_PHASE_FINE);
        cph_rfux++;
        rval = rfu_transfer(read_ioreg32(REG_SIODATA32_L));
        core_phase_leave(CORE_PHASE_FINE, &cph_rfut, cph_t);
