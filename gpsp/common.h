...
/* GBA palette entries are BGR555 (B 10-14, G 5-9, R 0-4).  Which 16-bit
 * layout we expand them into is a build-time choice, because every consumer
 * downstream -- the blend code in video.cc, the frontend blit -- just
 * inherits it.
 *
 * USE_PSP_RGB565_FORMAT emits the PSP GE's native 5650 (R 0-4, G 5-10,
 * B 11-15).  The default RGB565 layout is libretro's (R 11-15, G 5-10,
 * B 0-4), which the PSP frontend then had to swap BACK, per pixel, per
 * frame: 1071 us of a 16743 us budget spent undoing a swap this macro had
 * just done for free (ADR-0039).  Note it is also the CHEAPEST of the
 * three -- one mask, one shift, one or.
 *
 * video.cc needs NO change for this.  Its blend path separates a pixel into
 * 0G0R0B and clamps with SATR/SATB/OVFR/OVFB masks; R and B are both 5 bits
 * and sit in the two positions those masks already name, so swapping which
 * channel occupies which position permutes the mask NAMES and leaves every
 * constant, and all the arithmetic, identical.  G does not move at all
 * (bits 5-10 in both layouts). */
#if defined(USE_XBGR1555_FORMAT)
  #define convert_palette(value)  \
    (value & 0x7FFF)
#elif defined(USE_PSP_RGB565_FORMAT)
  #define convert_palette(value) \
    ((value & 0x1F) | ((value & 0x7FE0) << 1))
#else
  #define convert_palette(value) \
