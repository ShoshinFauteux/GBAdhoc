...
#define dma_write_iwram(type, tfsize)                                         \
  ...
  if (address##tfsize(iwram, type##_ptr & 0x7FFF)) {                          \
    /* ADR-0030: record only the FIRST tagged halfword of the transfer.       \
     * `alerts` is local to one dma_tf_loop call and the whole transfer costs \
     * exactly one flush no matter how many tagged halfwords it crosses, so   \
     * counting per halfword here would make the profile incommensurable with \
     * the CPU-store events (and swamp them: one code DMA is ~1900 writes). */\
    if (!(alerts & CPU_ALERT_SMC))                                            \
      smc_prof_note_iwram(type##_ptr);                                        \
...
#define dma_write_ewram(type, tfsize)                                         \
  ...
  if (address##tfsize(ewram, (type##_ptr & 0x3FFFF) + 0x40000)) {             \
    if (!(alerts & CPU_ALERT_SMC))     /* ADR-0030, once per transfer */      \
      smc_prof_note_ewram(type##_ptr);                                        \
    alerts |= CPU_ALERT_SMC;                                                  \
  }                                                                           \
...
u8 *load_gamepak_page(u32 physical_index)
{
   ...
   * frame-time spike to ROM paging instead of guessing. */
  gamepak_page_loads++;
  if (rom_blocks == 0)
